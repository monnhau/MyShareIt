...
public class NewsApi {
    ...
    public static void API_GET_POST_DETAIL(Context ctx, String link, HttpCallback httpCallback ){
      ...
      /*BEGIN-----------ShareIt Bien Tap Tin  Request-------------------------------------------------------------*/

      public static void API_GET_LIST_PARENT_CAT_SHAREIT(Context ctx, HttpCallback httpCallback){
          //tao 1 Calnback nho vao BaseOkhttp
          BaseOkHttp baseOkHttp = new BaseOkHttp.Builder()
                  .setHttpCallback(httpCallback)
                  .setContext(ctx)
    ...
    public static void API_GET_LIST_POST_SHAREIT(Context ctx,int catId,int page, HttpCallback httpCallback){
        //Tao 1 callback dua tren doi tuong Baseokhttp
        BaseOkHttp baseOkHttp = new BaseOkHttp.Builder()
                .setHttpCallback(httpCallback)
                .setContext(ctx)
        ...
        //Tao nhan vat chnh cua chung ta HttpClient
        OkHttpClient okHttpClient = BaseOkHttp.getOkHttpClient();
        String url = Define.API_GET_LIST_POST_SHAREIT+"/cat/"+catId+"/"+page;
        LogUtil.d("urlGetListPost", url);
        Request request = new Request.Builder()
                .url(url)
                .build();
    ...
    /*END-----------ShareIt Bien Tap Tin  Request----------------------------------------------------------*/
}
